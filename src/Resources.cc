 ...
 * lists for actions.
 */
ResourceHandler::ResourceHandler(Meawm *wa, struct waoptions *options) {
	...
	if ((homedir = getenv("HOME"))) {
		...
		configdir.append( "/.meawm" );
		::mkdir( configdir.c_str(), 0755 );

		configdir.append( "/styles" );
		::mkdir( configdir.c_str(), 0755 );
	}
	...
	if (options->rcfile) {
		...
		rc_file = new char[strlen(homedir) + strlen("/.meawmrc") + 1];
		sprintf(rc_file, "%s/.meawmrc", homedir);
	}
	...
	wacts.push_back(new StrComp("previousdesktop", &WaWindow::PreviousDesktop));
	...
	wacts.push_back(new StrComp("partcurrentdesktop", &WaWindow::PartCurrentDesktop));
	wacts.push_back(new StrComp("joinalldesktops", &WaWindow::JoinAllDesktops));
	wacts.push_back(new StrComp("partalldesktopsexceptcurrent", &WaWindow::PartAllDesktopsExceptCurrent));
	wacts.push_back(new StrComp("partcurrentjoindesktop", &WaWindow::PartCurrentJoinDesktop));

#ifdef ENABLE_TILING
	wacts.push_back(new StrComp("mergewithwindow",&WaWindow::CloneMergeWithWindow));
	wacts.push_back(new StrComp("vertmergewithwindow", &WaWindow::VertMergeWithWindow));
	wacts.push_back(new StrComp("horizmergewithwindow", &WaWindow::HorizMergeWithWindow));
	wacts.push_back(new StrComp("explode", &WaWindow::Explode)); wacts.push_back(new StrComp("mergedtofront", &WaWindow::ToFront));
	wacts.push_back(new StrComp("unmerge", &WaWindow::UnMergeMaster)); wacts.push_back(new StrComp("setmergemode", &WaWindow::SetMergeMode));
	...
	racts.push_back(new StrComp("menuremapfocused", &WaScreen::MenuRemapFocused));
	...
	racts.push_back(new StrComp("carrousel", &WaScreen::Carrousel));
	racts.push_back(new StrComp("compositor", &WaScreen::Compositing));
	racts.push_back(new StrComp("showing", &WaScreen::Showing));
	...
	racts.push_back(new StrComp("viewportright", &WaScreen::MoveViewportRight));
	...
	racts.push_back(new StrComp("viewportrelativemove", &WaScreen::ViewportRelativeMove));
	racts.push_back(new StrComp("viewportfixedmove", &WaScreen::ViewportFixedMove));
	racts.push_back(new StrComp("startviewportmove", &WaScreen::ViewportMove));
	...
	racts.push_back(new StrComp("pointerrelativewarp", &WaScreen::PointerRelativeWarp));
	racts.push_back(new StrComp("pointerfixedwarp", &WaScreen::PointerFixedWarp));
	racts.push_back(new StrComp("gotodesktop", &WaScreen::GoToDesktop));
	racts.push_back(new StrComp("nextdesktop", &WaScreen::NextDesktop));
	racts.push_back(new StrComp("previousdesktop", &WaScreen::PreviousDesktop));
...
/**
 * @fn	LoadConfig(WaScreen *meawm)
 * @brief Reads config file
 *
 * Reads all configuration resources for common for all screens from the
 * config file.
 *
 * @param meawm Pointer to Meawm object
 */
void ResourceHandler::LoadConfig(Meawm *meawm, struct waoptions *options) {
	std::string pathenv( "/sbin:" );
	XrmValue value;
	char *value_type;
	...
	WARNING << "Opened rcfile `" << rc_file << "' for reading" << endl;
	...
	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		...
		if (token) meawm->screenmask |= (1L << atoi(token));
		while ((token = strtok(NULL, ", \t")))
			meawm->screenmask |= (1L << atoi(token));
	} else
		meawm->screenmask = (1L << 0) | (1L << 2) | (1L << 3);
	...
	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		...
		meawm->pathenv = new char[strlen(path) + strlen(sdir) + 7];
		sprintf(meawm->pathenv, "PATH=%s:%s", sdir, path);
		...
		sprintf(meawm->pathenv, "PATH=%s:%s", DEFAULTSCRIPTDIR, path);
		pathenv += DEFAULTSCRIPTDIR;
	}

	pathenv += ":";
	pathenv += path;

	std::cout << "PATH=" << pathenv << "\n";
	::setenv( "PATH", pathenv.c_str(), true );

	sprintf(rc_name, "doubleClickInterval");
	sprintf(rc_class, "DoubleClickInterval");

	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		if (sscanf(value.addr, "%lu", &meawm->double_click) != 1)
			meawm->double_click = 300;
	} else {
		meawm->double_click = 300;
	}

	if (meawm->double_click > 999) meawm->double_click = 999;

	sprintf(rc_name, "appLauncherEnabled");
	sprintf(rc_class, "AppLauncherEnabled");
	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		if (! strncasecmp("true", value.addr, value.size)) {
			options->applauncher = true;
		} else {
			options->applauncher = false;
		}
	} else {
		options->applauncher = false;
	}

	sprintf(rc_name, "compositorEnabled");
	sprintf(rc_class, "CompositorEnabled");
	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		if (! strncasecmp("true", value.addr, value.size)) {
			options->compositor = true;
		} else {
	...
	sprintf(rc_name, "compositor");
	sprintf(rc_class, "Compositor");
	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		delete [] meawm->compositor;
		meawm->compositor = environment_expansion(__m_wastrdup(value.addr));
	}

	sprintf(rc_name, "appLauncher");
	sprintf(rc_class, "AppLauncher");
	if (XrmGetResource(database, rc_name, rc_class, &value_type, &value)) {
		delete [] meawm->applauncher;
		meawm->applauncher = environment_expansion(__m_wastrdup(value.addr));
	}
...
void ResourceHandler::LoadMenus(WaScreen *wascreen) {
	...
	if (! (file = fopen(wascreen->config.menu_file, "r"))) {
		WARNING << "can't open menufile `" << wascreen->config.menu_file << "' for reading" << endl;
		return;
	}
...
void ResourceHandler::ReadDatabaseTexture(const char *rname, const char *rclass,
  WaTexture *texture, unsigned long default_pixel, WaImageControl *ic)
{
	XrmValue value;
	char *value_type;
...
#ifdef PIXMAP
	if (rclass && rname && value.addr) {
		std::cout << "RCLASS=" << rclass << "\n";
		std::cout << "RNAME=" << rname << "\n";
	...
	if ((texture->getTexture() & WaImage_Pixmap)) {
		...
		std::cout << "RCLASS='" << rclass << "' '" << pixmapclass << "'\n";
		std::cout << "RNAME='" << rname << "' '" << pixmapname << "'\n";
		std::cout << "VALUE=" << value.addr << "\n";

		if (XrmGetResource(database, pixmapname, pixmapclass, &value_type, &value)) {

  			if (! (image = imlib_load_image(value.addr))) {
				if (value.addr[0] != '/') {
					sprintf(pixmap_path, "%s/%s",
						ic->getWaScreen()->config.style_file, value.addr);
		std::cout << "PIXMAP-PATH=" << pixmap_path << "\n";

					if (! (image = imlib_load_image(pixmap_path))) {
						sprintf(pixmap_path, "%s/default/%s",
							DEFAULTTHEMESDIR, value.addr);
	...
	if (XrmGetResource(database, colorname, colorclass, &value_type, &value))
		opacity = atoi(value.addr);
	else
		opacity = 0;
	...
	if (opacity > 0 && opacity < 255) {
		...
		xformat = XRenderFindFormat(ic->getDisplay(), PictFormatType | PictFormatDepth | PictFormatAlphaMask, &Rpf, 0);
		alphaPixmap = XCreatePixmap(ic->getDisplay(), ic->getDrawable(), 1, 1, 8);
		alphaPicture = XRenderCreatePicture(ic->getDisplay(), alphaPixmap, xformat, CPRepeat, &Rpa);
		XRenderFillRectangle(ic->getDisplay(), PictOpSrc, alphaPicture, &clr, 0, 0, 1, 1);
...
void ResourceHandler::ParseAction(const char *_s, list<StrComp *> *comp,
	...
	if (! act_tmp->param || *(act_tmp->param) == '\0') {
		if (act_tmp->winfunc && (
			...
			act_tmp->winfunc == (WwActionFn) &WaWindow::PartCurrentJoinDesktop))
#ifdef ENABLE_TILING
			act_tmp->winfunc == (WwActionFn) &WaWindow::VertMergeWithWindow ||
			act_tmp->winfunc == (WwActionFn) &WaWindow::HorizMergeWithWindow ||
			act_tmp->winfunc == (WwActionFn) &WaWindow::CloneMergeWithWindow ||
			act_tmp->winfunc == (WwActionFn) &WaWindow::SetMergeMode))
#endif
		{
			WARNING "`" << token << "' action must have a parameter" << endl;
			delete act_tmp;
			delete [] line;
			if (s) delete [] s; s = NULL;
			return;
