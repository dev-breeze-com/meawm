...
void WaMenu::Build(WaScreen *screen) {
    ...
    for (y = 0, lasttype = 0; it != item_list.end(); ++it) {
        ...
        (*it)->id = XCreateWindow(display, frame, x, y, width,
			  ...
			  &attrib_set);
        meawm->window_table.insert(make_pair((*it)->id, (*it)));
        (*it)->dy = y;
        y += (*it)->height + bw * 2;
#ifdef XFT
	...
	/*
	title = "root menu"
	XChangeProperty(display.XDisplay(), frame,
		net_wm_visible_name, utf8_string,
		8, PropModeReplace,
...
void WaMenu::Lower(void) {
	std::cerr << "void WaMenu::Lower(void)\n";
	std::cerr.flush();
...
WaMenuItem::~WaMenuItem(void) {
    ...
    if (id) {
        menu->meawm->window_table.erase(id);
    ...
    if (((menu->x + menu->width) > 0 && menu->x < menu->wascreen->width) && 
        ...
        if (type == MenuTitleType) {
            ...
            if (menu->wascreen->config.db) {
                ...
                if (menu->ptitle)
                    XSetWindowBackgroundPixmap(menu->display, id, menu->ptitle);
                else
...
void WaMenuItem::UnmapMenu(XEvent *, WaAction *, bool focus) {
    if (! in_window) return;
    if (menu->meawm->eh->move_resize != EndMoveResizeType) return;
    menu->Unmap(focus);
}
...
void WaMenuItem::Func(XEvent *e, WaAction *ac) {
    ...
    if ((func_mask & MenuWFuncMask) &&
        ...
        if ((it = menu->meawm->window_table.find(func_win)) !=
            menu->meawm->window_table.end()) {
...
void WaMenuItem::Move(XEvent *e, WaAction *) {
    ...
    if (menu->meawm->eh->move_resize != EndMoveResizeType) return;
    menu->meawm->eh->move_resize = MoveType;
    move_resize = true;
    ...
    if (XGrabPointer(menu->display, id, true, ButtonReleaseMask |
                     ...
                     menu->wascreen->id, menu->meawm->move_cursor,
                     CurrentTime) != GrabSuccess) {
        move_resize = false;
        menu->meawm->eh->move_resize = EndMoveResizeType;
        return;
    }
    ...
    for (;;) {
        menu->meawm->eh->EventLoop(
            menu->meawm->eh->menu_viewport_move_return_mask, &event);
        switch (event.type) {
            case MotionNotify:
            ...
            case EnterNotify:
                if (menu->wascreen->west->id == event.xcrossing.window ||
                    ...
                    menu->meawm->eh->HandleEvent(&event);
            ...
            case KeyRelease:
                ...
                menu->meawm->eh->HandleEvent(&event);
                if (menu->meawm->eh->move_resize != EndMoveResizeType) break;
...
void WaMenuItem::EvAct(XEvent *e, EventDetail *ed, list<WaAction *> *acts)
{
    ...
    if (menu->meawm->eh->move_resize != EndMoveResizeType)
    ...
    for (; it != acts->end(); ++it) {
        if (eventmatch(*it, ed)) {
            if ((*it)->delay.tv_sec || (*it)->delay.tv_usec) {
                ...
                menu->meawm->timer->AddInterrupt(i);
            } else {
                if ((*it)->exec)
...
void WaMenuItem::Carrousel(XEvent *e, WaAction *ac) {
    menu->wascreen->Carrousel(e, ac);
}
void WaMenuItem::Compositing(XEvent *e, WaAction *ac) {
    menu->wascreen->Compositing(e, ac);
}
void WaMenuItem::Showing(XEvent *e, WaAction *ac) {
    menu->wascreen->Showing(e, ac);
}
