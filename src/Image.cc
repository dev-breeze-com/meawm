 ...
 * fit into the Meawm Window Manager project. Except these small
...
Pixmap WaImage::render_pixmap(WaTexture *texture) {
    ...
    if ((texture->getTexture() & WaImage_Scale) || (texture->getTexture() & WaImage_Stretch))
    ...
    if (! cache->empty()) {
        ...
        for (; it != cache->end(); ++it) {
            if (((*it)->width == width) &&
                ((*it)->height == height) &&
                ((*it)->texture == texture) &&
                ((*it)->pixel1 == c1->getPixel())) { // Ambiguous else
...
#ifdef RENDER
        retp = xrender(pixmap, width, height, texture, parent, src_x, src_y, dest);
#else // !RENDER
        retp = pixmap;
#endif // RENDER
