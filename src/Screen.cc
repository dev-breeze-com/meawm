...
/**
 * @fn    WaScreen(Display *d, int scrn_number, Meawm *wa)
 ...
 */
WaScreen::WaScreen(Display *d, int scrn_number, Meawm *wa) : WindowObject(0, RootType)
{
	...
	if (meawm->wmerr) {
	   cerr << "meawm: warning: another window manager is running on " <<
	...
	compositing_on = false;
	carrousel_desktop = false;
	...
	meawm->window_table.insert(make_pair(id, this));
	...
	CreateColors();
	RenderCommonImages();
	XDefineCursor(display, id, meawm->session_cursor);
	...
	wamenu_list.push_back(new MergeMenu(CloneMergeType,
	    "Merge with", "__mergelist__"));
	wamenu_list.push_back(new MergeMenu(VertMergeType,
	    "Merge vertically with", "__mergelist_vertically__"));
	wamenu_list.push_back(new MergeMenu(HorizMergeType,
	    "Merge horizontally with", "__mergelist_horizontally__"));
	...
	for (i = 0; i < (int) nchild; ++i) {
	   ...
	   if (status && (! attr.override_redirect) &&
	       ...
	       if (net->IsSystrayWindow(children[i])) {
	           if (! (meawm->FindWin(children[i], SystrayType))) {
	               XGrabServer(display);
	               ...
	               meawm->window_table.insert(make_pair(children[i], stw));
	       ...
	       else if ((meawm->window_table.find(children[i]))
	                == meawm->window_table.end()) {
	           WaWindow *newwin = new WaWindow(children[i], this);
	           if (meawm->FindWin(children[i], WindowType)) {
...
void WaScreen::UpdateWorkarea(void) {
	...
	for (; it != strut_list.end(); ++it) {
	   WindowObject *wo = meawm->FindWin((*it)->window,
...
void WaScreen::GetWorkareaSize(int *x, int *y, int *w, int *h) {
	...
	*h = current_desktop->workarea.height;
	...
	if (meawm->xinerama && meawm->xinerama_info) {
	   XQueryPointer(display, id, &win, &win, &px, &py, &i, &i, &ui);
	   for (i = 0; i < meawm->xinerama_info_num; ++i) {
	       if (px > meawm->xinerama_info[i].x_org &&
	           px < (meawm->xinerama_info[i].x_org +
	                 meawm->xinerama_info[i].width) &&
	           py > meawm->xinerama_info[i].y_org &&
	           py < (meawm->xinerama_info[i].y_org +
	                 meawm->xinerama_info[i].height)) {
	           int diff = meawm->xinerama_info[i].x_org - *x;
	           int xt = meawm->xinerama_info[i].width;
	           if (diff > 0) {
	               *w -= diff;
	               *x = meawm->xinerama_info[i].x_org;
	           } else xt += diff;
	           if (*w > xt) *w = xt;

	           diff = meawm->xinerama_info[i].y_org - *y;
	           xt = meawm->xinerama_info[i].height;
	           if (diff > 0) {
	               *h -= diff;
	               *y = meawm->xinerama_info[i].y_org;
	           } else xt += diff;
	           if (*h > xt) *h = xt;
	           break;
	       }
...
void WaScreen::ViewportFixedMove(XEvent *, WaAction *ac) {
	...
	int mask = XParseGeometry(ac->param, &x, &y, &w, &h);
...
void WaScreen::ViewportRelativeMove(XEvent *, WaAction *ac) {
	...
	//int mask = XParseGeometry(ac->param, &x, &y, &w, &h);
	(void) XParseGeometry(ac->param, &x, &y, &w, &h);
...
void WaScreen::ViewportMove(XEvent *e, WaAction *) {
	...
	if (meawm->eh->move_resize != EndMoveResizeType) return;
	meawm->eh->move_resize = MoveOpaqueType;
	...
	XGrabPointer(display, id, true, ButtonReleaseMask | ButtonPressMask |
	            ...
	            GrabModeAsync, GrabModeAsync, None, meawm->move_cursor,
	            CurrentTime);
	...
	for (;;) {
	   ...
	   meawm->eh->EventLoop(meawm->eh->menu_viewport_move_return_mask,
	                         &event);
	   switch (event.type) {
	       case MotionNotify: {
	       ...
	       case KeyRelease:
	           ...
	           meawm->eh->HandleEvent(&event);
	           if (meawm->eh->move_resize != EndMoveResizeType) break;
...
void WaScreen::EndMoveResize(XEvent *, WaAction *) {
	meawm->eh->move_resize = EndMoveResizeType;
}
...
/**
 * @fn    Carrousel(XEvent *, WaAction *)
 * @brief Carrousel window manager
 *
 * Show current desktop by showing windows in carrousel mode.
 *
 * @param ac WaAction object
 */
void WaScreen::Carrousel(XEvent *e, WaAction *ac) {
	if ( ac->param ) {
		Carrousel((unsigned int) atoi(ac->param));
	}
}

void WaScreen::Carrousel(unsigned int flag)
{
	std::list<WaWindow*>::const_iterator it;
	float nb_windows = wawindow_list.size();
	CarrouselRect crattrs;

	carrousel_desktop = flag;

	for (it = wawindow_list.begin(); it != wawindow_list.end(); ++it) {
		WaWindow *w = (*it);
		if ((w->desktop_mask & (1L << current_desktop->number))) {
			nb_windows++;
		}
	}

	float coord_x_slice = width / nb_windows;
	float scale_slice = 1.0 / nb_windows;

	crattrs.windows = nb_windows;
	crattrs.curidx = 0;
	crattrs.scale = 0.5;
	crattrs.x = 5;

	fprintf( stderr, "void WaScreen::Carrousel(%d)\n", flag );
	fflush( stderr );
	...
	for (it = wawindow_list.begin(); it != wawindow_list.end(); ++it) {
		...
		if ((w->desktop_mask & (1L << current_desktop->number))) {
			...
			bool at_halway_point = crattrs.curidx++ >= nb_windows/2;

			if (crattrs.scale >= 1.0)
				crattrs.scale = 1.0;

			if ( carrousel_desktop ) {
				w->TakeSnapshot();
				w->ShowInCarrousel( crattrs );

				if ( at_halway_point ) {
					crattrs.scale -= scale_slice;
				} else {
					crattrs.scale += scale_slice;
				}
			} else {
			...
			crattrs.x += coord_x_slice;
		}
	}
}

/**
 * @fn    Showing(XEvent *, WaAction *)
 * @brief Showing window manager
 *
 * Showing desktop by hiding or showing windows on current desktop.
 *
 * @param ac WaAction object
 */
void WaScreen::Showing(XEvent *, WaAction *ac) {
	if (ac->param)
		Showing((unsigned int) atoi(ac->param));
}

void WaScreen::Showing(unsigned int flag) {

	showing_desktop = flag;
	...
	for (; it != wawindow_list.end(); ++it) {
		...
		if (w->desktop_mask & (1L << current_desktop->number)) {
			...
			} else if (! w->hidden ) { // && ! w->flags.hidden) 
				w->Hide( true );
			}
		}
	}
...
/**
 * @fn    Compositing(XEvent *, WaAction *)
 * @brief Compositing manager
 *
 * Start or stop the compositing manager.
 *
 * @param ac WaAction object
 */
void WaScreen::Compositing(XEvent *, WaAction *ac) {
	if (ac->param)
		Compositing((unsigned int) atoi(ac->param));
}

void WaScreen::Compositing(unsigned int flag) {
	compositing_on = flag;
	meawm->compositing( flag );
}
...
void WaScreen::GoToDesktop(unsigned int number) {
	...
	if (dit != desktop_list.end() && *dit != current_desktop) {
	   ...
	   if (meawm->eh) oldf = meawm->eh->focused;
	   XSetInputFocus(display, id, RevertToPointerRoot, CurrentTime);
	   ...
	   if (oldf) {
	       WaWindow *ww = (WaWindow *) meawm->FindWin(oldf,
	                                                   WindowType);
	       if (ww) {
	           if (ww->desktop_mask &
	               (1L << current_desktop->number)) {
...
void WaScreen::EvAct(XEvent *e, EventDetail *ed, list<WaAction *> *acts) {
	...
	if (meawm->eh->move_resize != EndMoveResizeType)
	...
	for (; it != acts->end(); ++it) {
		...
		//std::cout << "void WaScreen::SEvAct(XEvent *, WaAction *ac)\n";
		//std::cout.flush();

	   if (eventmatch(*it, ed)) {
	       if ((*it)->delay.tv_sec || (*it)->delay.tv_usec) {
	           Interrupt *i = new Interrupt(*it, e, id);
	           meawm->timer->AddInterrupt(i);
	       }
	       else {
	           if ((*it)->exec)
 ...
 */
ScreenEdge::ScreenEdge(WaScreen *wascrn, int x, int y, int width, int height, int type) : WindowObject(0, type)
{
	XSetWindowAttributes attrib_set;
	...
	wa->meawm->net->wXDNDMakeAwareness(id);
}
...
void ScreenEdge::SetActionlist(list<WaAction *> *list)
{
	...
	if (! actionlist->empty()) {
	   ...
	   wa->meawm->window_table.insert(make_pair(id, this));
	}
}
...
{
	if (! actionlist->empty())
		wa->meawm->window_table.erase(id);
