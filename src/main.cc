 ...
 * @brief Main function for Meawm window manager
 *
 * Parses command line options. Creates a new meawm object.
...
int main(int argc, char **argv) {
    ...
    options.menufile = NULL;
	options.actionfile = NULL;
	options.stylefile = NULL;
    options.rcfile = NULL;
	options.display = NULL;

    // This means that cout and cerr will become fully buffered on their own.
    ...
    Meawm *meawm = new Meawm(argc, argv, &options);

    meawm->eh->EventLoop(meawm->eh->empty_return_mask, &e);
...
void help(void) {
    ...
    cout << "Meawm - an X11 window manager designed for maximum efficiency" <<
        endl << endl;
    ...
    cout << "   --version                Output version information and exit"
         << endl << endl;
    cout << "Report bugs to <dev@breezeos.com>." << endl;
}
