...
NetHandler::NetHandler(Meawm *wa) {
	...
	net_showing_desktop = XInternAtom(display, "_NET_SHOWING_DESKTOP", false);
	net_compositing = XInternAtom(display, "_NET_COMPOSITING", false);
	...
	meawm_net_wm_state_decor =
		XInternAtom(display, "_MEAWM_NET_WM_STATE_DECOR", false);
	meawm_net_wm_state_decortitle =
		XInternAtom(display, "_MEAWM_NET_WM_STATE_DECOR_TITLE", false);
	meawm_net_wm_state_decorhandle =
		XInternAtom(display, "_MEAWM_NET_WM_STATE_DECOR_HANDLE", false);
	meawm_net_wm_state_decorborder =
		XInternAtom(display, "_MEAWM_NET_WM_STATE_DECOR_BORDER", false);
	
	meawm_net_maximized_restore =
		XInternAtom(display, "_MEAWM_NET_MAXIMIZED_RESTORE", false);
	meawm_net_virtual_pos =
		XInternAtom(display, "_MEAWM_NET_VIRTUAL_POS", false);
	meawm_net_wm_desktop_mask =
		XInternAtom(display, "_MEAWM_NET_WM_DESKTOP_MASK", false);

#ifdef ENABLE_TILING
	meawm_net_wm_merged_to =
		XInternAtom(display, "_MEAWM_NET_WM_MERGED_TO", false);
	meawm_net_wm_merged_type =
		XInternAtom(display, "_MEAWM_NET_WM_MERGED_TYPE", false);
	meawm_net_wm_merge_order =
		XInternAtom(display, "_MEAWM_NET_WM_MERGE_ORDER", false);
	meawm_net_wm_merge_atfront =
		XInternAtom(display, "_MEAWM_NET_WM_MERGE_ATFRONT", false);
#endif // ifdef ENABLE_TILING

	meawm_net_showing = XInternAtom(display, "_MEAWM_NET_SHOWING", false);
	meawm_net_compositing = XInternAtom(display, "_MEAWM_NET_COMPOSITING", false);
	meawm_net_restart = XInternAtom(display, "_MEAWM_NET_RESTART", false);
	meawm_net_shutdown = XInternAtom(display, "_MEAWM_NET_SHUTDOWN", false);
...
void NetHandler::GetState(WaWindow *ww) {
	...
	if (validatedrawable(ww->id)) {
		if (XGetWindowProperty(display, ww->id, wm_state, 0L, 1L, false,
			...
			&items_left, (unsigned char **) &data) == Success && items_read) {
			ww->state = *data;
			XFree(data);
		}
	} else ww->deleted = true;
...
void NetHandler::GetWmState(WaWindow *ww) {
	...
	if (status == Success && items_read) {
		for (i = 0; i < items_read; i++) {
			...
			else if (data[i] == meawm_net_wm_state_decor) decor = true;
			else if (data[i] == meawm_net_wm_state_decortitle) title = true;
			else if (data[i] == meawm_net_wm_state_decorhandle) handle = true;
			else if (data[i] == meawm_net_wm_state_decorborder) border = true;
		}
	}
	if (decor) {
...
void NetHandler::SetWmState(WaWindow *ww) {
	...
	if (validatedrawable(ww->id)) {
		...
		if (ww->flags.max) {
			...
			XChangeProperty(display, ww->id, meawm_net_maximized_restore,
							XA_CARDINAL, 32, PropModeReplace,
							(unsigned char *) data2, 6);
		} else
			XDeleteProperty(display, ww->id, meawm_net_maximized_restore);
		
		data[i++] = meawm_net_wm_state_decor;
		if (ww->flags.title) data[i++] = meawm_net_wm_state_decortitle;
		if (ww->flags.handle) data[i++] = meawm_net_wm_state_decorhandle;
		if (ww->flags.border) data[i++] = meawm_net_wm_state_decorborder;
 ...
 *
 * Sets _NET_SUPPORTED hint to the atoms Meawm supports.
 *
...
void NetHandler::SetSupported(WaScreen *ws) {
	...
	data[i++] = net_carrousel_desktop;
	data[i++] = net_showing_desktop;
	data[i++] = net_compositing;
	...
	XChangeProperty(
		display, ws->id, net_supported, XA_ATOM, 32,
		PropModeReplace, (unsigned char *) data, i
	);
}
...
void NetHandler::SetClientList(WaScreen *ws) {
	...
	list<WaWindow *>::iterator it = ws->wawindow_list_map_order.begin();
	for (; it != ws->wawindow_list_map_order.end(); ++it) {
		data[i++] = (*it)->id;
	}

	XChangeProperty(
		display, ws->id, net_client_list, XA_WINDOW, 32,
		PropModeReplace, (unsigned char *) data, i
	);
	delete [] data;
}
...
void NetHandler::SetClientListStacking(WaScreen *ws) {
	...
	for (; it != ws->aab_stacking_list.rend(); ++it) {
		wc = (WaChildWindow *) meawm->FindWin(*it, FrameType);
...
void NetHandler::GetClientListStacking(WaScreen *ws) {
	...
	if (XGetWindowProperty(display, ws->id, net_client_list_stacking,
		...
		items_read) {
		for (i = 0; i < items_read; i++) {
			WaWindow *ww = (WaWindow *) meawm->FindWin(data[i], WindowType);
...
void NetHandler::GetActiveWindow(WaScreen *ws) {
	...
	if (XGetWindowProperty(display, ws->id, net_active_window, 0L,
		...
		for (i = items_read - 1; i >= 0; i--) {
			...
			ww = (WaWindow *) meawm->FindWin(data[i], WindowType);
			if (ww) {
				ws->wawindow_list.remove(ww);
				ws->wawindow_list.push_front(ww);
				if (i == 0)
...
void NetHandler::GetVirtualPos(WaWindow *ww) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id)) {
		if (XGetWindowProperty(display, ww->id, meawm_net_virtual_pos,
...
void NetHandler::SetVirtualPos(WaWindow *ww) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id))
		XChangeProperty(display, ww->id, meawm_net_virtual_pos, XA_INTEGER,
						32, PropModeReplace, (unsigned char *) data, 2);
	else ww->deleted = true;
	XUngrabServer(display);
...
/**
 * @fn	SetCarrouselDesktop(WaScreen *ws)
 * @brief Writes carrousel desktop hint
 *
 * Sets _NET_CARROUSEL_DESKTOP hint.
 *
 * @param ws WaScreen object
 */
void NetHandler::SetCarrouselDesktop(WaScreen *ws) {
	ulong data[1];
	data[0] = ws->carrousel_desktop;
	XChangeProperty(
		display, ws->id, net_carrousel_desktop, XA_CARDINAL, 32,
		PropModeReplace, (unsigned char *) data, 1
	);
}

/**
 * @fn	GetCarrouselDesktop(WaScreen *ws)
 * @brief Reads carrousel desktop hint
 *
 * Reads _NET_CARROUSEL_DESKTOP hint and sets desktop accordingly.
 *
 * @param ws WaScreen object
 */
void NetHandler::GetCarrouselDesktop(WaScreen *ws) {
	long *data;

	if (XGetWindowProperty(display, ws->id, net_carrousel_desktop, 0L, 1L, 
...
/**
 * @fn	SetCompositing(WaScreen *ws)
 * @brief Writes compositing flag
 *
 * Sets _NET_COMPOSITING hint.
 *
 * @param ws WaScreen object
 */
void NetHandler::SetCompositing(WaScreen *ws) {
	ulong data[1];
	data[0] = ws->compositing_on;
	XChangeProperty(
		display, ws->id, net_compositing, XA_CARDINAL, 32,
		PropModeReplace, (unsigned char *) data, 1
	);
}

/**
 * @fn	GetCompositing(WaScreen *ws)
 * @brief Reads compositing flag
 *
 * Reads _NET_COMPOSITING hint and sets desktop accordingly.
 *
 * @param ws WaScreen object
 */
void NetHandler::GetCompositing(WaScreen *ws) {
	long *data;

	if (XGetWindowProperty(display, ws->id, net_compositing, 0L, 1L, 
	   false, XA_CARDINAL, &real_type, &real_format, &items_read, &items_left, 
	   (unsigned char **) &data) == Success && items_read)
	{
		ws->Compositing(data[0]);
		XFree(data);
	}
}

/**
 * @fn	SetShowingDesktop(WaScreen *ws)
 * @brief Writes showing desktop hint
 *
 * Sets _NET_SHOWING_DESKTOP hint.
 *
 * @param ws WaScreen object
 */
void NetHandler::SetShowingDesktop(WaScreen *ws) {
	ulong data[1];
	data[0] = ws->showing_desktop;
	XChangeProperty(display, ws->id, net_showing_desktop, XA_CARDINAL, 32,
		PropModeReplace, (unsigned char *) data, 1);
}

/**
 * @fn	GetShowingDesktop(WaScreen *ws)
 * @brief Reads showing desktop hint
 *
 * Reads _NET_SHOWING_DESKTOP hint and sets desktop accordingly.
 *
 * @param ws WaScreen object
 */
void NetHandler::GetShowingDesktop(WaScreen *ws) {
	long *data;

	if (XGetWindowProperty(display, ws->id, net_showing_desktop, 0L, 1L, 
	   false, XA_CARDINAL, &real_type, &real_format, 
	   &items_read, &items_left, 
	   (unsigned char **) &data) == Success && items_read)
	{
...
void NetHandler::SetWorkarea(WaScreen *ws) {
	...
	std::list<Desktop *>::iterator it = ws->desktop_list.begin();
...
void NetHandler::wXDNDMakeAwareness(Window window) {
	...
	XChangeProperty(meawm->display, window, xdndaware, XA_ATOM,
...
void NetHandler::wXDNDClearAwareness(Window window) {
	XDeleteProperty (meawm->display, window, xdndaware);
}
 ...
 *
 * Sets _MEAWM_NET_WM_DESKTOP_MASK hint to the current desktop mask
 * of the window.
 *
 * @param ww WaWindow object
 */
void NetHandler::SetDesktopMask(WaWindow *ww) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id)) {
		XChangeProperty(display, ww->id, meawm_net_wm_desktop_mask,
						XA_CARDINAL, 32, PropModeReplace,
 ...
 *
 * Reads _NET_WM_DESKTOP and _MEAWM_NET_WM_DESKTOP_MASK hints and sets
...
void NetHandler::GetDesktop(WaWindow *ww) {
	...
	if (validatedrawable(ww->id)) {
		...
		if (XGetWindowProperty(display, ww->id, meawm_net_wm_desktop_mask,
 ...
 *
 * Reads _MEAWM_NET_WM_MERGED_TO hint to see if window is merged with
 * another window.
 *
 * @param ww WaWindow object
 */
void NetHandler::GetMergedState(WaWindow *ww) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id)) {
		if (XGetWindowProperty(display, ww->id, meawm_net_wm_merged_to, 0L,
			...
			mwin = *data;
			XFree(data);
			if (XGetWindowProperty(display, ww->id, meawm_net_wm_merged_type,
								   0L, 1L, false, XA_CARDINAL, &real_type,
								   &real_format, &items_read, &items_left, 
								   (unsigned char **) &data) == Success && 
				items_read) {
	...
	if (mwin) {
		WaWindow *master = (WaWindow *)
			ww->meawm->FindWin(mwin, WindowType);
		if (master) {
 ...
 *
 * Sets _MEAWM_NET_WM_MERGED_TO hint to the master window. If window isn't
 * merged the _MEAWM_NET_WM_MERGED_TO hint is removed.
 *
 * @param ww WaWindow object
 */
void NetHandler::SetMergedState(WaWindow *ww) {
	...
	if (validatedrawable(ww->id)) {
		if (ww->master) {
			data[0] = ww->master->id;
			XChangeProperty(display, ww->id, meawm_net_wm_merged_to,
							XA_WINDOW, 32, PropModeReplace,
							(unsigned char *) data, 1);
			data[0] = ww->mergetype;
			XChangeProperty(display, ww->id, meawm_net_wm_merged_type,
							XA_CARDINAL, 32, PropModeReplace,
							(unsigned char *) data, 1);
		} else
			XDeleteProperty(display, ww->id, meawm_net_wm_merged_to);
	} else
		ww->deleted = true;
	XUngrabServer(display);
}
 ...
 *
 * Sets _MEAWM_NET_WM_MERGE_ORDER hint to the current order of merged
...
void NetHandler::SetMergeOrder(WaWindow *ww) {
	...
	if (validatedrawable(ww->id)) {
		if (i) {
			XChangeProperty(display, ww->id, meawm_net_wm_merge_order,
							XA_WINDOW, 32, PropModeReplace,
							(unsigned char *) data, i);
		} else
			XDeleteProperty(display, ww->id, meawm_net_wm_merge_order);
	} else
		ww->deleted = true;
	XUngrabServer(display);
}
 ...
 *
 * Reads _MEAWM_NET_WM_MERGE_ORDER hint and sorts merged windows in the same
 * order.
 *
 * @param ww WaWindow object
 */
void NetHandler::GetMergeOrder(WaWindow *ww) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id)) {
		XGetWindowProperty(display, ww->id, meawm_net_wm_merge_order, 0L,
 ...
 *
 * Sets _MEAWM_NET_WM_MERGED_ATFRONT hint to the window that is currently
...
void NetHandler::SetMergeAtfront(WaWindow *ww, Window win) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id)) {
		XChangeProperty(display, ww->id, meawm_net_wm_merge_atfront,
						XA_WINDOW, 32, PropModeReplace,
						(unsigned char *) data, 1);
	} else
		ww->deleted = true;
 ...
 *
 * Reads _MEAWM_NET_WM_MERGED_ATFRONT hint and sets window indicated by
...
void NetHandler::GetMergeAtfront(WaWindow *ww) {
	...
	XGrabServer(display);
	if (validatedrawable(ww->id)) {
		XGetWindowProperty(display, ww->id, meawm_net_wm_merge_atfront, 0L,
						   1L, false, XA_WINDOW, &real_type, &real_format,
						   &items_read, &items_left,
						   (unsigned char **) &data);
	} else
