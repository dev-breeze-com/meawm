...
/**
 * @fn    EventHandler(Meawm *wa)
 * @brief Constructor for EventHandler class
 *
 * Sets meawm and rh pointers. Creates menu move return mask, window
 * move/resize return mask and empty return mask sets.
 *
 * @param wa Pointer to meawm object
 */
EventHandler::EventHandler(Meawm *wa) {
    meawm = wa;
    rh = meawm->rh;
...
void EventHandler::EventLoop(set<int> *return_mask, XEvent *event) {    
    for (;;) {
        XNextEvent(meawm->display, event);
    ...
    switch (event->type) {
        ...
        case Expose:
            if (event->xexpose.count == 0) {
                while (XCheckTypedWindowEvent(meawm->display,
        ...
        case MapRequest:
            ...
            ed->type = event->type;
            XQueryPointer(meawm->display, event->xmaprequest.parent,
        ...
        default:
#ifdef SHAPE
            if (event->type == meawm->shape_event) {
                XShapeEvent *e = (XShapeEvent *) event;
                WaWindow *ww = (WaWindow *)
                    meawm->FindWin(e->window, WindowType);
                if (ww && meawm->shape)
...
#ifdef RANDR
            if (event->type == meawm->randr_event) {
                XRRScreenChangeNotifyEvent *e =
                    (XRRScreenChangeNotifyEvent *) event;
                WaScreen *ws = (WaScreen *)
                    meawm->FindWin(e->window, RootType);
                if (ws) {
                    ws->width = e->width;
                    ws->height = e->height;
                    ws->RRUpdate();
			...
			ButtonPressMask | // End of window manager mask
			/*
			PropertyChangeMask | // Start of compton composite manager mask
			StructureNotifyMask |
			*/
...
void EventHandler::EvProperty(XPropertyEvent *e) {
    ...
    if (e->state == PropertyDelete) {
        if (e->atom == meawm->net->net_wm_strut) {
            if ((ww = (WaWindow *) meawm->FindWin(e->window, WindowType))) {
    ...
    } else if (e->atom == meawm->net->net_wm_strut) {
        if ((ww = (WaWindow *) meawm->FindWin(e->window, WindowType)))
            meawm->net->GetWmStrut(ww);
    } else if (e->atom == XA_WM_NAME) {
        if ((ww = (WaWindow *) meawm->FindWin(e->window, WindowType))) {
            meawm->net->GetXaName(ww);
...
#ifdef RENDER
    else if (e->atom == meawm->net->xrootpmap_id) {

        if (WaScreen *ws = (WaScreen *) meawm->FindWin(e->window, RootType)) {
            meawm->net->GetXRootPMapId(ws);
...
void EventHandler::EvExpose(XExposeEvent *e) {
    if (WindowObject *wo = meawm->FindWin(e->window, LabelType | ButtonType |
...
void EventHandler::EvFocus(XFocusChangeEvent *e) {
    ...
    if (e->type == FocusIn && e->window != focused) {
        ww = (WaWindow *) meawm->FindWin(e->window, WindowType);
        if (ww) {
            ww->actionlist =
        ...
        } else if ((ws = (WaScreen *) meawm->FindWin(e->window, RootType)))
            ws->focus = true;
        
        if ((ww2 = (WaWindow *) meawm->FindWin(focused, WindowType))) {
            ww2->actionlist =
            ...
            ww2->UpdateGrabs();
            ww2->UnFocusWin();
            if (! ww) meawm->net->SetActiveWindow(ww2->wascreen, NULL);
        }
        focused = e->window;
    }
}
...
void EventHandler::EvConfigureRequest(XConfigureRequestEvent *e) {
    ...
    if ((wo = meawm->FindWin(e->window, WindowType | DockAppType))) {
...
void EventHandler::EvMapRequest(XMapRequestEvent *e) {
    ...
    if (WaWindow *ww = (WaWindow *) meawm->FindWin(e->window, WindowType)) {
        if (ww->flags.hidden) ww->UnMinimize(NULL, NULL);
    }
    else if (WaScreen *ws =
             (WaScreen *) meawm->FindWin(e->parent, RootType)) {
        if (ws->net->IsSystrayWindow(e->window)) {
            if (! meawm->FindWin(e->window, SystrayType)) {
                ...
                meawm->window_table.insert(make_pair(e->window, stw));
...
void EventHandler::EvUnmapDestroy(XEvent *e) {
    ...
    if ((wo = meawm->FindWin((e->type == UnmapNotify)?
                              e->xunmap.window:
        ...
        if (wo->type == WindowType) {
            ...
            meawm->window_table.erase(stw->id);
...
void EventHandler::EvClientMessage(XEvent *e, EventDetail *ed) {
    ...
    if (e->xclient.message_type == meawm->net->net_active_window) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType)))
            ww->RaiseFocus(NULL, NULL);
    }
    else if (e->xclient.message_type == meawm->net->net_wm_name) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
            meawm->net->GetNetName(ww);
            if (ww->wascreen->config.db) {
                ww->title->Render();
                ww->label->Render();
            } else
    ...
    else if (e->xclient.message_type == meawm->net->wm_change_state) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
    ...
    else if (e->xclient.message_type == meawm->net->net_wm_desktop) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
            if ((unsigned int) e->xclient.data.l[0] == 0xffffffff ||
             ...
             meawm->net->meawm_net_wm_desktop_mask) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
    ...
    else if (e->xclient.message_type == meawm->net->net_wm_state) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
            bool max_done = false;
            for (int i = 1; i < 3; i++) {
                if ((unsigned long) e->xclient.data.l[i] ==
                    meawm->net->net_wm_state_sticky) {
                    ...
                    switch (e->xclient.data.l[0]) {
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->net_wm_maximized_vert ||
                           (unsigned long) e->xclient.data.l[i] ==
                           meawm->net->net_wm_maximized_horz) {
                    if (max_done) break;
                    max_done = true;
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->net_wm_state_above ||
                           (unsigned long) e->xclient.data.l[i] ==
                           meawm->net->net_wm_state_stays_on_top) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->net_wm_state_below ||
                           (unsigned long) e->xclient.data.l[i] ==
                           meawm->net->net_wm_state_stays_at_bottom) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->net_wm_state_skip_taskbar) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->net_wm_state_fullscreen) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->meawm_net_wm_state_decor) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->meawm_net_wm_state_decortitle) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->meawm_net_wm_state_decorhandle) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
                        ...
                        case _NET_WM_STATE_TOGGLE:
                           ...
                           meawm->net->meawm_net_wm_state_decorborder) {
                    switch (e->xclient.data.l[0]) {
                        case _NET_WM_STATE_REMOVE:
    ...
    else if (e->xclient.message_type == meawm->net->xdndenter ||
             e->xclient.message_type == meawm->net->xdndleave) {
        if (e->xclient.message_type == meawm->net->xdndenter) {
            e->type = EnterNotify;
            ed->type = EnterNotify;
        } else {
            e->type = LeaveNotify;
        ...
        if (WaScreen *ws = (WaScreen *) meawm->FindWin(e->xclient.window,
                                                        RootType)) {
    ...
    else if (e->xclient.message_type == meawm->net->net_desktop_viewport) {
        if (WaScreen *ws = (WaScreen *) meawm->FindWin(e->xclient.window,
                                                        RootType))
            ws->MoveViewportTo(e->xclient.data.l[0], e->xclient.data.l[1]);
    }
    else if (e->xclient.message_type == meawm->net->net_close_window) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window, WindowType)))
            ww->Close(NULL, NULL);
    }
    else if (e->xclient.message_type == meawm->net->net_current_desktop) {
        if (WaScreen *ws = (WaScreen *) meawm->FindWin(e->xclient.window,
                                                        RootType))
            ws->GoToDesktop(e->xclient.data.l[0]);
    }
    else if (e->xclient.message_type == meawm->net->net_moveresize_window) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
    ...
    else if (e->xclient.message_type == meawm->net->net_wm_moveresize) {
        if ((ww = (WaWindow *) meawm->FindWin(e->xclient.window,
                                               WindowType))) {
    ...
    else if (e->xclient.message_type == meawm->net->meawm_net_carrousel) {
        if ((ws = (WaScreen *) meawm->FindWin(e->xclient.window, RootType))) {
            flag = ws->carrousel_desktop ? 0 : 1;
        	ws->Carrousel( flag );
		}
    }
    else if (e->xclient.message_type == meawm->net->meawm_net_compositing) {
        if ((ws = (WaScreen *) meawm->FindWin(e->xclient.window, RootType))) {
            flag = ws->compositing_on ? 0 : 1;
        	ws->Compositing( flag );
		}
    }
    else if (e->xclient.message_type == meawm->net->meawm_net_showing) {
        if ((ws = (WaScreen *) meawm->FindWin(e->xclient.window, RootType))) {
            flag = ws->showing_desktop ? 0 : 1;
        	ws->Showing( flag );
		}
    }
    else if (e->xclient.message_type == meawm->net->meawm_net_restart) {
        restart(NULL);
    }
    else if (e->xclient.message_type == meawm->net->meawm_net_shutdown) {
        quit(EXIT_SUCCESS);
    }
}
...
void EventHandler::EvAct(XEvent *e, Window win, EventDetail *ed) {
    ...
    if ((it = meawm->window_table.find(win)) !=
        meawm->window_table.end()) {
        wo = (*it).second;

        meawm->timer->ValidateInterrupts(e);
