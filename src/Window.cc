...
extern "C" {
	...
	void compton_add_window(Window);
	void compton_set_opacity(Window, float);
}
#endif
...
WaWindow::~WaWindow(void)
{
	...
	if (transient_for) {
		if (transient_for == wascreen->id) {
			...
			if ((hit = meawm->window_table.find(transient_for)) !=
...
{
	if (size.min_width > attrib.width)
		attrib.width = size.min_width;

	if (size.min_height > attrib.height)
		attrib.height = size.min_height;
	...
	old_attrib.x = - 0xffff;
	old_attrib.y = - 0xffff;
	old_attrib.width = - 0xffff;
	old_attrib.height - 0xffff;
}
...
/**
 * @fn	TakeSnapshot()
 * @brief Take Snapshot of window
 *
 * Take a snapshot of the window to initialize carrousel mode.
 */
void WaWindow::TakeSnapshot()
{
}
 ...
 * @brief Show window
 *
 * Show window in carrousel mode.
 */
void WaWindow::ShowInCarrousel(const CarrouselRect& crattrs)
{
}
 ...
 *
 * Restore window with snapshotted attributes
 */
void WaWindow::RestoreSnapshot()
{
}
...
void WaWindow::Move(XEvent *e, WaAction *) {
	...
	if (validatedrawable(w->id)) {
		if (XGrabPointer(display, (w->mapped && !w->hidden) ? id:
						 ...
						 None, meawm->move_cursor, CurrentTime)
			!= GrabSuccess) {
			move_resize = false;
			meawm->eh->move_resize = EndMoveResizeType;
			return;
	...
	XUngrabServer(display);
	for (;;) {
		meawm->eh->EventLoop(meawm->eh->moveresize_return_mask, &event);
		switch (event.type) {
			case MotionNotify:
			...
			case UnmapNotify:
				...
				meawm->eh->EvUnmapDestroy(&event);
				break;
			case ConfigureRequest:
				if (event.xconfigurerequest.window != id)
					meawm->eh->EvConfigureRequest(&event.xconfigurerequest);
				break;
			case MapRequest:
				map_ev = new XEvent;
				*map_ev = event;
...
bool WaWindow::_MoveOpaque(XEvent *e, int saved_w, int saved_h, int px,
	...
	if (meawm->eh->move_resize != EndMoveResizeType) return false;
	sx = nx = attrib.x;
	sy = ny = attrib.y;
...
void WaWindow::Resize(XEvent *e, int how) {
	...
	meawm->eh->move_resize = ResizeType;
	move_resize = true;
	...
	if (validatedrawable(id)) {
		if (XGrabPointer(display, (mapped && !hidden) ? id: wascreen->id, true,
						 ...
						 PointerMotionMask | EnterWindowMask |
						 LeaveWindowMask, GrabModeAsync, GrabModeAsync,
						 None, (how > 0) ? meawm->resizeright_cursor:
						 meawm->resizeleft_cursor, CurrentTime)
			!= GrabSuccess) {
			move_resize = false;
			meawm->eh->move_resize = EndMoveResizeType;
			return;
...
void WaWindow::ResizeOpaque(XEvent *e, int how) {
	...
	meawm->eh->move_resize = ResizeOpaqueType;
	move_resize = true;
...
void WaWindow::MoveResize(XEvent *e, WaAction *ac) {
	...
	if (meawm->eh->move_resize != EndMoveResizeType || ! ac->param) return;
	width = attrib.width;
...
void WaWindow::Merge(WaWindow *child, int mtype) {
	...
	if (mtype == CloneMergeType) {
		...
		if (meawm->eh) child->ToFront(NULL, NULL);
	}
...
void WaWindow::MergeTo(XEvent *e, int mtype) {
	WaWindow *wt = (WaWindow *) meawm->FindWin(e->xany.window, WindowType);
	if (wt)
		Merge(wt, mtype);
}
 ...
 */
void WaWindow::EvAct(XEvent *e, EventDetail *ed, list<WaAction *> *acts, int etype) {
	XEvent fev;
...
WaChildWindow::WaChildWindow(WaWindow *wa_win, Window parent, int type) :
	...
	pressed = false;
	int create_mask =
		CWOverrideRedirect|CWBorderPixel|CWEventMask|CWColormap;
	...
	switch (type) {
		case FrameType:
			...
			attrib.height =
				wa->attrib.height + wa->title_w + wa->handle_w +
				wa->border_w * 2;
			break;
		case LabelType:
		...
		case LGripType:
			...
			attrib_set.cursor = wa->meawm->resizeleft_cursor;
			break;
		case RGripType:
			f_texture = &wascreen->wstyle.g_focus;
			u_texture = &wascreen->wstyle.g_unfocus;
			create_mask |= CWCursor;
			attrib_set.cursor = wa->meawm->resizeright_cursor;
			break;
	}

	id = XCreateWindow(
		display, parent, attrib.x, attrib.y,
		attrib.width, attrib.height, 0, CopyFromParent,
		CopyFromParent, CopyFromParent, create_mask, &attrib_set
	);
...
void WaChildWindow::Render(void) {
	...
	int pos_x;
	int pos_y;
	XTranslateCoordinates(display, id, wa->wascreen->id, 0, 0, &pos_x, &pos_y, &wd);
...
void WaWindow::Carrousel(XEvent *e, WaAction *ac) {
	wascreen->Carrousel(e, ac);
}
void WaWindow::Compositing(XEvent *e, WaAction *ac) {
	wascreen->Compositing(e, ac);
}
void WaWindow::Showing(XEvent *e, WaAction *ac) {
	wascreen->Showing(e, ac);
}
